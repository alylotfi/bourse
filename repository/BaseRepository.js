class BaseRepository {
  ...
  async list (options) {
    return new Promise(async (resolve, reject) => {
      try {
        const result = await this._findAndCountAll(options)
        options.pagination.count = result.count
        resolve({
          items: result.rows,
  ...
  async bulkInsert (options = {}) {
    return new Promise(async (resolve, reject) => {
      try {
        resolve(await this.model.bulkCreate(options))
      } catch (e) {
        // TODO logging
        console.log(e)
  ...
  __getLimit (pagination) {
    return pagination.pageSize || constants.pagination.defaults.pageSize
  }
